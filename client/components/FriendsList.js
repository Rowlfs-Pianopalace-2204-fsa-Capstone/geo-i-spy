...
export default function FriendsList({ navigation }) {
  ...
  return (
    <ScrollView style={tw`flex-1 pt-12 px-8`}>
      <View style={tw`flex-1 items-center`}>
        <Text style={tw`p-2 px-1 py-2 font-bold text-2xl`}>Your friends!</Text>
        <TouchableOpacity>
          <View style={tw`bg-blue-400 rounded-lg m-2 items-center`}>
            <Text style={tw`font-bold m-4`}>Add new friends</Text>
          </View>
        </TouchableOpacity>
      </View>
        ...
        {dummyData.map((friend) => {
          return (
            <View key={friend.id} style={tw`pb-10  border-2 mb-10`}>
              <TouchableOpacity style={tw`h-30 w-30`}>
